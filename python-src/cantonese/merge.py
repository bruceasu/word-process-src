    ...
    for line in f_first:
        ...
        try:
            if kv[0] in dict_map:
                #if kv[0] == u'怠':
                #    print line
                #    print kv[0], kv[0] in dict_map, dict_map[kv[0]],len(dict_map[kv[0]])
                for x in xrange(0, len(dict_map[kv[0]])):
                    #print kv[0], kv[0] in dict_map, dict_map[kv[0]],len(dict_map[kv[0]]), dict_map[kv[0]][x]
                    print >> f_out_main, (u"%s\t%s:%s" % (kv[0], kv[1], dict_map[kv[0]][x])).encode(encoding)
...
def main():
    ext = 'zyoy.single.mapped.txt'
    # ext = u'../data/wubi.2codes.txt'
