...
import sys
import merge as m
import make_short_for_my as ms
...
def merge(first, last_file='data/rain.txt', charset="utf-8"):
    ...
    merged_file = 'out/hyly.single.merged.txt'
...
if __name__ == '__main__':
    encoding = 'utf-8'
    py = u"data/带调拼音-he.txt"
    # merge
    ext = "data/kuaishou.txt"
    merged = merge(py, ext, encoding)
    # make short
    make_short(merged, encoding)
    sort_file('out\\hyly.single.merged-out.txt')
    sort_file('out\\hyly.single.merged-full.txt')
    sort_file('out\\hyly.single.merged-ci.txt')
