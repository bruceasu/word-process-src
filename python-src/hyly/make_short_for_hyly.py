...
def make_short(infile, coding='utf-8'):
        ...
        3. 先处理前1600字，如果有空二简的，填到二简中，如过三简不超过2个，可以填充三简。
        4. 再处理前5000字，如果有空三简的，填到三简中。
        5. 超过5000字的，只有全码。
        """
        ...
        for (w, k1, k2) in set_500:
                ...
                if key_syllable not in code_set:
                        ...
                        full.append(w + "\t" + cc_full)
                elif key_syllable in code_set and code_set[key_syllable] < 2:
        ...
        for (k, ws) in set_other.iteritems():
                ...
                if length == 1:
                        ...
                        elif (ws[0], k[0:2], k[2:]) in set_500:
                                result.append("%s\t%s\t6" % (ws[0], k))
                        elif (ws[0], k[0:2], k[2:]) in set_1600:
                                result.append("%s\t%s\t7" % (ws[0], k))
                        elif (ws[0], k[0:2], k[2:]) in set_3700:
                                result.append("%s\t%s\t8" % (ws[0], k))
                        elif (ws[0], k[0:2], k[2:]) in set_5000:
                        ...
                        for i in xrange(1, length):
                                ...
                                if ws[0] in one_set:
                                        result.append("%s\t%s\t%s" % (ws[0], new_code, (i + 10)))
                                elif (ws[0], k[0:2], k[2:]) in set_500:
                                        result.append("%s\t%s\t%s" % (ws[i], new_code, (i + 10)))
                                elif (ws[0], k[0:2], k[2:]) in set_1600:
                                        result.append("%s\t%s\t%s" % (ws[i], new_code, (i + 10)))
                                elif (ws[0], k[0:2], k[2:]) in set_3700:
                                        result.append("%s\t%s\t%s" % (ws[i], new_code, (i + 10)))
