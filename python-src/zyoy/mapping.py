...
def get_map_of_zyoy():
    ...
    w = {
        ...
        "r": "f",  # a
        "e": "d",
        "v": "h",  # yu , 同u合并，kut,kvt, gun, gvn 会混，但方言中，两组音也是混的，关系不大。
        "u": "w",  # 长u，于短u是互补的。
        "i": "y",  # 长i，于短i是互补的。
        "o": "l",
        "!": "s",  # eo    eo, eu是互补的。
...
def main():
    f1 = '..\..\words-and-phrases-data\cantonese-data\zyoy.single.txt'
    f2 = "..\..\words-and-phrases-data\cantonese-data\zyoy.single.mapped.txt"
    encoding = 'utf-8'
    length = len(sys.argv)
    if length > 2:
        f1 = sys.argv[1]
